...
function handleServerRequest(request, response) {
    ...
    switch(urlParts.pathname){
        ...
        case '/set_miner_payout_level':
            ...
            break;
        case '/get_miner_public_transaction_settings':
            handleGetMinerPublicTransactionSettings(urlParts, response);
            break;
        case '/set_miner_public_transaction_settings':
            handleSetMinerPublicTransactionSettings(urlParts, response);
            break;
...
// get public transaction settings
function handleGetMinerPublicTransactionSettings(urlParts, response){
    response.writeHead(200, {
        'Access-Control-Allow-Origin': '*',
        'Cache-Control': 'no-cache',
        'Content-Type': 'application/json'
    ...
    // Return current miner payout level
    redisClient.get(config.coin + ':publictransactionsettings:' + address, function(error, value){
        /*if (error){
            response.end(JSON.stringify({status: 'Unable to get the public transaction settings from database'}));
            return;
        }*/
        if (error){
            response.end(JSON.stringify({status: 'done', settings: 0}));
            return;
        }
        response.end(JSON.stringify({status: 'done', settings: value}));
    });
}
 
// set public transaction settings
function handleSetMinerPublicTransactionSettings(urlParts, response){
    response.writeHead(200, {
        'Access-Control-Allow-Origin': '*',
        'Cache-Control': 'no-cache',
        'Content-Type': 'application/json'
    ...
    var address = urlParts.query.address;
    var ip = urlParts.query.ip;
    var settings = urlParts.query.settings;
 
    // Check the minimal required parameters for this handle.
    if (ip === undefined || address === undefined || settings === undefined) {
        response.end(JSON.stringify({status: 'Parameters are incomplete'}));
        return;
    }
    ...
    if (settings != 0 && settings != 1) {
        response.end(JSON.stringify({status: 'The public transactions settings are invalid'}));
        return;
    }
    ...
    minerSeenWithIPForAddress(address, ip, function (error, found) {
        ...
        redisClient.set(config.coin + ':publictransactionsettings:' + address, settings, function(error, value){
            if (error){
                response.end(JSON.stringify({status: 'An error occurred when updating the public transaction settings in our database'}));
                return;
            }
 
            log('info', logSystem, 'Updated public transaction settings for ' + address + ' to: ' + settings);
            response.end(JSON.stringify({status: 'done'}));
        });
    });
}
